  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        data = data.map(
            tweet => tweet.extended_entities.map(
                entities => entities.media.map(
                    tweets => tweets.map(
                        tweet =>  {
                            if (tweet.type === 'video') {
                                vids["size"] = tweet.sizes.large 
                                vids["id"] = tweet.id_str
                                vids["type"] = tweet.type
                                vids["link"] = tweet.video_info.variants.map(
                                    vid => vid.url.includes(tweet.sizes.large.h)
                                )
                            }
                        }
                    )
        ...
        // data = data.map(dataArr => dataArr.map( tweet => { if (tweet.type === 'video') { vids.push(tweet)} } ) )
        console.log(vids)
      });
  }, [username]);
